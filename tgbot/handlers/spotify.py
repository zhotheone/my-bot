...
async def find_song(message: types.Message):
    ...
    process = subprocess.run(['spotdl', '--threads', '3', '--preload', '--format', 'm4a', 'download', f'{" ".join(name)}'])
    files = os.listdir()
    for file in files:
        mus = InputFile(file)
...
async def down_song(message: types.Message):
    ...
    if not os.path.isdir(str(message.chat.id)):
        os.mkdir(str(message.chat.id))
    os.chdir(str(message.chat.id))
    process = subprocess.run(['spotdl', '--threads', '3', '--preload', '--format', 'm4a', 'download', f'{message.text}'])
    files = os.listdir()
    for file in files:
        mus = InputFile(file)
        await message.answer_audio(mus)
